	...
	<table class="table table-striped" style="margin-top: 10px;">
		...
		<tbody>
			<c:forEach var="in" items="${inList }">
				<tr>
					<td><fmt:formatDate value="${in.date_time }"
							pattern="yyyy년MM월 dd일 HH시 mm분" /></td>
					<td><a href="#" id="emp_infoLink"
						onclick="window.open('/emp/info?employee_id=${in.employee_id}', 'employee_info');">
							${in.employee_id }</a></td>
				</tr>
			</c:forEach>
	...
	<ul class="pagination" id="pagination">
		<c:choose>
			<c:when test="${pageVO.startPage > pageVO.pageBlock}">
				<li class="page-item"><a
					href="/emp/inEmp_list?pageNum=${pageVO.startPage - pageVO.pageBlock}"
		...
		<c:forEach var="i" begin="${pageVO.startPage}" end="${pageVO.endPage}"
			...
			<li
				...
				<a href="/emp/inEmp_list?pageNum=${i}" class="page-link">${i}</a>
			</li>
		</c:forEach>
