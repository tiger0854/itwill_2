...
//	http://localhost:8088/customer/customerList
	...
	public void customerGET(Model model,PageVO pageVO, HttpServletRequest request)throws Exception {
		...
		// 게시글 개수 가져오기
		int count = pService.customerList(); // 요 동작만 각자 페이지에 맞게 수정하면 됨!!
...
//	http://localhost:8088/customer/customerList
	...
	public String searchList(Model model,PageVO pageVO, HttpServletRequest request)throws Exception {
		...
		// 게시글 개수 가져오기
		int count = pService.serchCusname(pageVO); // 요 동작만 각자 페이지에 맞게 수정하면 됨!!
		...
		model.addAttribute("pageVO", pageVO);
		//================================페이징 처리를 위한 값 받아오기 동작========================================
		model.addAttribute("customerList",cService.serchCusname(pageVO));
